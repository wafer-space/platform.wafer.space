...
# 5. Configure environment variables (edit .env - see section below)
nano .env
...
5. Writes `DATABASE_URL` directly to `/home/django/platform.wafer.space/.env`
6. Sets proper file permissions (640, owner django:django)
...
The `.env` file contains all secrets and configuration. Edit it as the django user:
...
DJANGO_ALLOWED_HOSTS=platform.wafer.space
DJANGO_ADMIN_URL=admin-custom-path-change-this/

# Database (auto-generated by 03-setup-database.sh)
DATABASE_URL=postgres://platform_wafer_space:PASSWORD@localhost:5432/platform_wafer_space
...
# OAuth Providers (see GitHub/GitLab/Google OAuth apps)
